...
export function useMainLoopModel(): ModelName {
  ...
  // Check if we're in llama mode - if so, use getMainLoopModel() which handles
  // the llama model properly. Otherwise use the default resolution.
  if (apiProvider === 'llama' && !mainLoopModel && !mainLoopModelForSession) {
