...
import { isLlamaModeEnabled } from '../utils/model/providers.js'
import {
...
function isManagedOAuthContext(): boolean {
  return (
    ...
    process.env.LOUKANIKODE_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  // Llama mode: local-only, no external auth.
  if (isLlamaModeEnabled()) return false
  ...
  const is3P =
    isEnvTruthy(process.env.LOUKANIKODE_USE_BEDROCK) ||
